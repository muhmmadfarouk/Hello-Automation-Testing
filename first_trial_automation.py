...
class Create_User(unittest.TestCase):  # create the test case
  ...
  def test_login(self):   # go to login page
    self.type_user= self.driver.find_element_by_xpath('//*[@id="mat-input-0"]')   #find the username field in login Page
    self.type_user.send_keys(user)   #write the username                                             
    self.type_password= self.driver.find_element_by_xpath('//*[@id="password"]')   #find the Password field in login Page
    ...
    self.type_pwd = self.driver.find_element_by_xpath('//*[@id="mat-input-6"]')   #find the password field in create user Page
    self.type_pwd.send_keys(pwd)       #write the data of password in create user page
    self.type_mail= self.driver.find_element_by_xpath('//*[@id="mat-input-7"]')   #find the mails field in create user Page
